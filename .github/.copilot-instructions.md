...
**CRITICAL**: Use correct line continuation for each shell:

- **Unix/Linux/macOS** (bash/zsh): Use backslash `\`
  ```yaml
  run: |
    cmake -B build \
      -DFLAG=VALUE \
...
Mixing these will cause silent failures or syntax errors in CI.
